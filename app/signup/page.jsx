...
export default function Signup() {
  ...
  const handleSignup = () => {
    axios.post(`${process.env.NEXT_PUBLIC_BACKEND_URL}/auth/register`, {
      ...
      axios.post(`${process.env.NEXT_PUBLIC_BACKEND_URL}/auth/login`, { email, password }).then((response) => {
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex flex-col items-center">
        <h1 className="text-3xl text-white mb-6">Signup</h1>
        <input
          type="email"
          placeholder="Email"
        ...
        />
        <button onClick={handleSignup} className="p-2 mt-4 bg-red-500 hover:bg-red-700 text-white rounded-md w-72">
          Signup
        </button>
      </div>
    </div>
